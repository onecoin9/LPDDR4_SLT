...
namespace Hsg.BLL.Model
{
    ...
    struct FanInfo
    {
        public TempCtrState state { get; internal set; }
        public AbnormalCode StateCode { get; internal set; }
        internal sbyte stateCount;
        public ushort value { get; internal set; }// 异常值
    }

    public partial class HightTempCtrTeam : StateMechine<TempCtrState, TempCtrEvent>, TempCtrTeam
    {
        ...
        private int _heatbeatTimer = TimerTask.INVALID_TIMER_ID;
        ...
        //private bool _startFromWrokMode;// 是否从工作模式重新开始
        //  private bool[] _dutEnableFlag;
        // private sbyte[] _dutTempStateCount;// 状态计数
        ...
        private bool[] _dutTempEnaleFlags;
        public HightTempCtrTeam(int instanceId, Action<TempCtrState> onStateChange,
                                          ...
                                          Action<int, float> onDutsValueChange) : base("TempCtrTeam" + (instanceId + 1).ToString())
        {
            ...
            _dutTempEnaleFlags = new bool[TempCtrBoardConfig.DUTS_COUNT];
            ...
            if(_config.TempCtrInitAllEnable)
            {
                for(int i=0;i<_dutTempEnaleFlags.Length;i++)
                {
                    _dutTempEnaleFlags[i] = true;
                }
            }
        ...
        protected override void NotifyStateChange(TempCtrState state)
        {
            switch (state)
            {
                ...
                case TempCtrState.STATE_WORKING:
                    PostEventAsync(TempCtrEvent.EVT_GET_TEMP);
        ...
        private void RegisterActionHandle()
        {
            ...
            RegisterStateHandle(TempCtrState.STATE_INIT, TempCtrEvent.EVT_RESET_EVT, DoActionStartHightTempMode);
            //  RegisterStateHandle(TempCtrState.STATE_INIT, TempCtrEvent.EVT_IDLE_HEATBEAT, DoActionIdleHeatbeat);
            RegisterStateHandle(TempCtrState.STATE_INIT, TempCtrEvent.EVT_EXIT, DoActionDestory);


            // RegisterStateHandle(TempCtrState.STATE_PREPARE, TempCtrEvent.EVT_INIT, DoActionInitPrepareState);
            RegisterStateHandle(TempCtrState.STATE_PREPARE, TempCtrEvent.EVT_SWITCH_ON_TEMP, DoActionSwitchOnTemp);
            RegisterStateHandle(TempCtrState.STATE_PREPARE, TempCtrEvent.EVT_SET_FAN_SPEED, DoActionSetFunSpeed);
            RegisterStateHandle(TempCtrState.STATE_PREPARE, TempCtrEvent.EVT_SET_TEMP, DoActionSetTemp);
            ...
            RegisterStateHandle(TempCtrState.STATE_PREPARE, TempCtrEvent.EVT_GET_TEMP, DoActionWaitTempStability);
            ...
            RegisterStateHandle(TempCtrState.STATE_WORKING, TempCtrEvent.EVT_GET_TEMP, DoActionCheckTemp);
            RegisterStateHandle(TempCtrState.STATE_WORKING, TempCtrEvent.EVT_REFRESH_ABNORMAL, DoActionRefreshTemp);
            RegisterStateHandle(TempCtrState.STATE_WORKING, TempCtrEvent.EVT_EXIT, DoActionDestory);
            RegisterStateHandle(TempCtrState.STATE_WORKING, TempCtrEvent.EVT_RESET_EVT, DoActionStartHightTempMode);
            ...
            //RegisterStateHandle(TempCtrState.STATE_WORKING, TempCtrEvent.EVT_START_WORK, /*DoActionRetryStartWork*/DoActionStartWork);

            // RegisterStateHandle(TempCtrState.STATE_ABNORMAL, TempCtrEvent.EVT_INIT, DoActionInitAbnormalState);
            RegisterStateHandle(TempCtrState.STATE_ABNORMAL, TempCtrEvent.EVT_START_HIGHT_TEMP_MODE, DoActionStartHightTempMode);
            RegisterStateHandle(TempCtrState.STATE_ABNORMAL, TempCtrEvent.EVT_RESET_EVT, DoActionStartHightTempMode);
        ...
        private void DoPrepareStateInit()
        {
            ...
            PostEventAsync(TempCtrEvent.EVT_SWITCH_ON_TEMP);
        }
        ...
        private void DoAbnormalStateInit()
        {
            ...
            for (int i = 0; i < _dutTempStateList.Length; i++)
            {
                ...
                UpdateDutTempState(i, TempCtrState.STATE_ABNORMAL);
            }
        ...
        private TempCtrState DoActionStartHightTempMode(object param)
        {
            StopRetryTimer();
        ...
        private TempCtrState DoActionSwitch5V(object param)
        {
            _voltageStateInfo = "";
            _currentStateInfo = "";
            lock (_stageLock)
        ...
        private TempCtrState DoActionSwitchOnTemp(object param)
        {

            TempCtrRet ret = TempCtrRet.ok;
            if (_retryCount < MAX_RETRY_TM)// 因为需要等待返回ack 所以启动3秒等待重试
            {
                StartRetryTimer(2000, TempCtrEvent.EVT_SWITCH_ON_TEMP);
                ret = _clientConn.SwitchOnTempCtr();
                ...
                {
                    Hlog.W(Mod, "send SwitchOnTempCtr faile.");
                }
            }
            else
            {
        ...
        private TempCtrState DoActionPrepareResetEvt(object param)
        {
            StopRetryTimer();
            _config.ReloadConfig();
            return DoActionSwitchOnTemp(param);
        }

        private TempCtrState DoActionSetTemp(object param)
        {
            TempCtrRet ret = TempCtrRet.ok;
            float targetTemp = _config.TargetTemp;
            float[] tempSetValue = new float[TempCtrBoardConfig.DUTS_COUNT];
            for (int i = 0; i < tempSetValue.Length; i++)
            {
                if (_config.UseDiffCompensator)
                {
                    tempSetValue[i] = _config.DutsTempCompensator[i];
                }
                else
                {
                    tempSetValue[i] = _config.DefaultTempCompensator;
                }
            }
            if (_retryCount < MAX_RETRY_TM)
            {
                StartRetryTimer(2000, TempCtrEvent.EVT_SET_TEMP);
                ret = _clientConn.SetTemp(targetTemp, tempSetValue, _dutTempEnaleFlags);
                if (ret == TempCtrRet.connect_fail)
                {
                    Hlog.W(Mod, "net discon");
                }
        ...
        {
            StartRetryTimer(2000, TempCtrEvent.EVT_GET_FAN_SEPPED); // 等待2S，后开始检查温度和风扇
            return TempCtrState.STATE_PREPARE;
        }
        ...
        private TempCtrState DoActionCheckTemp(object param)
        {
            TempCtrRet ret = TempCtrRet.ok;
        ...
        private TempCtrState DoActionRefreshTemp(object param)
        {
            ClearDutsTempAbnormalState();
            PostEventAsync(TempCtrEvent.EVT_GET_TEMP);
            return TempCtrState.STATE_WORKING;
        }
        //private TempCtrState DoActionRetryStartWork(object param)
        //{
        //    StopRetryTimer();
        //    _startFromWrokMode = true;
        //    return TempCtrState.STATE_PREPARE;
        //}
        ...
        private void IdleHeatbeatTimerStart()
        {
            lock (_waitLock)
            {
                ...
                {
                    _heatbeatTimer = StartTimer(3000, true, TempCtrEvent.EVT_IDLE_HEATBEAT);
                }
            }
        }
            ...
            switch (respId)
            {
                case TempProtocol.CMD_ON_OFF:
                    if (param != null && param.Length >= 1)
                    {
                        ...
                        {
                            if (!ClearWaitTimer(TempCtrEvent.EVT_SWITCH_ON_TEMP))
                            {
                                Hlog.D(Mod, "timeout resp CMD_ON_OFF:" + param[0]);
                                return;
                            }
                            PostEventAsync(TempCtrEvent.EVT_SET_FAN_SPEED);
                        }
                        Hlog.D(Mod, "resp CMD_ON_OFF:" + param[0]);
                    }
                    break;
                case TempProtocol.CMD_SET_TEMP:
                    if (param != null && param.Length >= 1)
                    {
                        if (param[0] == TempProtocol.STATE_CODE_OK)
                        {
                            if (!ClearWaitTimer(TempCtrEvent.EVT_SET_TEMP))
                            {
                                Hlog.D(Mod, "timeout resp CMD_SET_TEMP:" + param[0]);
                                return;
                            }
                            PostEventAsync(TempCtrEvent.EVT_CHECK_FAULT);
                        }
                        Hlog.D(Mod, "resp CMD_SET_TEMP:" + param[0]);
                    }
                    break;
                case TempProtocol.CMD_SET_FAN_SPEED:
                    if (param != null && param.Length >= 1)
                    {
                        if (param[0] == TempProtocol.STATE_CODE_OK)
                        {
                            ...
                            PostEventAsync(TempCtrEvent.EVT_SET_TEMP);
                        }
                ...
                case TempProtocol.CMD_QUERY_VOLTAGE:
                    ...
                    if (param != null && TempProtocol.DecodeVoltageParams(param, ref voltage, ref shuntVoltage))
                    {
                        ...
                        lock (_stageLock)
                        {
                            ...
                            else if (tb5v_line0 <= _config.VoltageCloseThreshold && tb5v_line1 <= _config.VoltageCloseThreshold)// 设置为低于0.1v 认为关闭
                            {
                                voltageLow = true;
                            }
                ...
                case TempProtocol.CMD_QUERY_FAN_SPEED:
                    if (param != null && TempProtocol.DecodeFanParams(param, ref fanSpeed))
                    {
                        ...
                        //PostEventAsync(TempCtrEvent.EVT_GET_FAN_SEPPED);
                        StartRetryTimer(1000, TempCtrEvent.EVT_GET_FAN_SEPPED);
                    }
                    break;
                case TempProtocol.CMD_QUERY_TEMP:
                    if (param != null && TempProtocol.DecodeTempParams(param, ref tempValues, _clientConn.TmpCompensators))
                    {
                        if (!ClearWaitTimer(TempCtrEvent.EVT_GET_TEMP))
                        {
                            Hlog.I(Mod, "timeout resp CMD_QUERY_TEMP");
                            return;
                        }
                        //    Hlog.I(Mod, "resp CMD_QUERY_TEMP:" + Tools.HexToString(param));
                        for (int i = 0; i < tempValues.Length; i++)
                        {
                            if (!_dutTempEnaleFlags[i])// 未执行工作的不管dut 温度状况
                            {
                                if(tempValues[i] >= _config.NormalTempHight && _config.NormalTempCheck)// 只有开启常温检测才认定
                                {
                                    SetDutStateCount(i, CountType.OverUpLimit, tempValues[i]);
                                }
                                else
                                {
                                    SetDutStateCount(i, CountType.Normal, tempValues[i]);
                                }
                            }
                            else if (tempValues[i] >= _config.WorkTempLow) // 转速低于目标速度
                            {
                                if (tempValues[i] >= _config.WorkTempHight)
                                {
                                    SetDutStateCount(i, CountType.OverUpLimit, tempValues[i]);
                                }
                                else
                            ...
                            {
                                SetDutStateCount(i, CountType.OverLowerLimit, tempValues[i]);
                            }
                        }
                        //PostEventAsync(TempCtrEvent.EVT_GET_TEMP);
                        StartRetryTimer(1500, TempCtrEvent.EVT_GET_TEMP);
                    }
                    break;
                case TempProtocol.CMD_SET_VOLTAGE:
                    ...
                    break;
                case TempProtocol.CMD_SET_TEMP_DETAIL:
                    if (param != null && param.Length >= 1)
                    {
                        if (param[0] == TempProtocol.STATE_CODE_OK)
                        {
        ...
        private void SetDutStateCount(int dutId, CountType type, float value)
        {
            if (dutId >= 0 && dutId < _dutTempStateList.Length)
            {
                if (_dutTempStateList[dutId].state == TempCtrState.STATE_PREPARE
                    || (!_AbnormalLock && _dutTempStateList[dutId].state == TempCtrState.STATE_ABNORMAL)
                   /* || (CurState == TempCtrState.STATE_PREPARE && _dutTempStateList[dutId].state == TempCtrState.STATE_ABNORMAL)*/) // 准备中
                {
                    if (type == CountType.Normal)//正常 累计
                    {
                        _dutTempStateList[dutId].stateCount++;
                        if (_dutTempStateList[dutId].stateCount >= MAX_STATE_COUNT) // 达到次数进入可工作状态
                        {
                            _dutTempStateList[dutId].stateCount = 0;
                            _dutTempStateList[dutId].StateCode = AbnormalCode.None;
                            _dutTempStateList[dutId].value = value;
                            _dutTempStateList[dutId].state = TempCtrState.STATE_WORKING;
                        }
                    }
                    else // 异常重置
                    {
                ...
                else if (_dutTempStateList[dutId].state == TempCtrState.STATE_WORKING)// 工作状态 抓异常
                {
                    if (type != CountType.Normal)// 异常累计
                    {
                        if (type == CountType.OverUpLimit)
                        {
                            _dutTempStateList[dutId].stateCount++;
                        }
                        else
                        ...
                        if (Math.Abs(_dutTempStateList[dutId].stateCount) >= MAX_STATE_COUNT) // 达到次数退出工作状态进入异常
                        {
                            if (_dutTempStateList[dutId].stateCount > 0)
                            {
                                _dutTempStateList[dutId].StateCode = AbnormalCode.OverUpLimit;
                            }
                            else
                            {
                                _dutTempStateList[dutId].StateCode = AbnormalCode.OverLowerLimit;
                            }
                            _dutTempStateList[dutId].value = value;
                            _dutTempStateList[dutId].stateCount = 0;
                            _dutTempStateList[dutId].state = TempCtrState.STATE_ABNORMAL;
                        }
                    }
                    else // 正常重置
                    {
        ...
        private void UpdateDutTempState(int dutId, TempCtrState state)
        {
            if (dutId >= 0 && dutId < _dutTempStateList.Length)
            {
                if (state == TempCtrState.STATE_ABNORMAL)
                {
                    _dutTempStateList[dutId].StateCode = AbnormalCode.SndFail;
                }
                else
                {
                    _dutTempStateList[dutId].StateCode = AbnormalCode.None;
